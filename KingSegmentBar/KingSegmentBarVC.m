...
@interface KingSegmentBarVC ()<KingSegmentBarDelegate, UIScrollViewDelegate>
@property (nonatomic, weak) UIScrollView *contentView;
@end

@implementation KingSegmentBarVC
#pragma mark lazy
- (KingSegmentBar *)segmentBar {
    if (!_segmentBar) {
        KingSegmentBar *segmentBar = [KingSegmentBar segmentBarWithFrame:CGRectZero];
...
- (void)setUpWithItems: (NSArray <NSString *>*)items childVCs: (NSArray <UIViewController *>*)childVCs {
    ...
    [self.segmentBar setItems:items];
...
- (void)updateWithConfig: (void(^)(KingSegmentBarConfig *config))configBlock {
    ...
    [self.segmentBar updateWithConfig:^(KingSegmentBarConfig *config) {
        ...
        self.view.backgroundColor = config.contentBGColor;
        self.contentView.bounces=config.bounces;
        self.segmentBar.frame = CGRectMake(0, 64, self.view.width, config.barHeight);
    }];
...
- (void)viewWillLayoutSubviews {
    ...
    [self.segmentBar setSelectIndex: self.segmentBar.selectIndex];
